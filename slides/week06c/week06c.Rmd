...
subtitle: "Election Polls, For Loops and If Statements<html><div style='float:left'></div><hr color='#EB811B' size=1px width=800px></html>"
...
.center[![:scale 70%](pollster2008.pdf)]
---
...
.center[![:scale 60%](EriksonWlezien.pdf)]
...
* Code without loop first by setting the counter to a specific value
]
---
...
```{r loop02,fig.align="center",tidy=F,warning=F,message=F,cache=T}
# our vector
values <- c(2, 4, 6)
...
results <- rep(NA, n)
# counter `i' will take values on 1, 2, ..., n in that order 
for (i in 1:n) {
...
```{r loop03,fig.align="center",tidy=F,message=F,cache=T}
values <- c(1, -1, 2)
results <- rep(NA, 3)
for (i in 1:3) {
    cat("iteration", i, "\n")
