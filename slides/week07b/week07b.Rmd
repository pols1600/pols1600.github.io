...
{{abline(lm(bivariate$Child.Mortality ~ log(bivariate$GDP)), col = "red", lwd = 2)}}
``` 
]
...
$$\textsf{SSR} = \sum_{i=1}^n \hat\epsilon_i^2 = \sum_{i=1}^n (Y_i - \widehat{Y_{i}})^2 = \sum_{i=1}^n (Y_i - \hat\alpha - \hat\beta X_i)^2$$
...
.font130[
* Enough math! R makes all that very easy to estimate with `lm()`
